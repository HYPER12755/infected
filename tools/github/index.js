...
function getRuntimeModuleRoot() {
    return path.resolve(process.cwd());
}
function resolveRuntimePath(relativeOrAbsolute) {
...
class GithubCommandsPlugin {
    constructor() {
        ...
        this.deregisterFns = [];
        this.defaultTimeoutMs = 180000;
        this.defaultGitTimeoutMs = 300000;
    }
    ...
    registerGenericTool(context) {
        ...
        const deregister = context.moduleManager.registerToolExecution(toolId, async (rawInput) => {
        ...
        }, toolTitle, description, inputSchema, this.manifest.id, true);
    ...
    registerGitCommandTool(context, commandName, commandDescription) {
        ...
        const inputSchema = commandName === 'commit'
            ...
            : SharedInputSchema;
        const deregister = context.moduleManager.registerToolExecution(toolId, async (rawInput) => {
            const baseEnhanced = commandName === 'commit' && rawInput.message
                ? {
                    ...rawInput,
                    flags: [...(rawInput.flags || []), '-m', rawInput.message],
                }
                : rawInput;
            const finalEnhanced = commandName === 'add' &&
    ...
    async executeGhCommand(baseTokens, rawInput, toolId) {
        ...
        try {
            ...
            if (!output.ok && !input.allowFailure) {
                throw new Error(`gh exited with code ${output.exitCode}. stderr: ${output.stderr || '(empty)'}`);
            }
            return output;
        }
        catch (error) {
    ...
    async executeGitCommand(baseTokens, rawInput, toolId) {
        ...
        try {
            ...
            if (!output.ok && !input.allowFailure) {
                throw new Error(`git exited with code ${output.exitCode}. stderr: ${output.stderr || '(empty)'}`);
            }
            return output;
        }
        catch (error) {
    ...
    normalizeInput(rawInput, fallbackTimeoutMs) {
        const cwd = this.resolveCwd(rawInput.cwd);
        const timeoutMs = this.clampNumber(rawInput.timeoutMs, 1000, 600000, fallbackTimeoutMs ?? this.defaultTimeoutMs);
    ...
    async killConflictingGitProcesses(cwd, excludePids = []) {
        ...
        for (const entry of entries) {
            ...
            if (!Number.isFinite(pid) || pid === process.pid || excludePids.includes(pid))
                continue;
            try {
    ...
    buildOutput(toolId, commandArgs, run, durationMs, binary = 'gh') {
    ...
    async runGh(commandArgs, cwd, timeoutMs, stdin) {
