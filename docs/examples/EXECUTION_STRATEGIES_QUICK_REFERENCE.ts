...
function getStrategy(commandType: string, factory: ExecutionStrategyFactory) {
  switch (commandType) {
    case 'interactive':
...
async function executeBatch(commands: string[], strategyMode: 'foreground' | 'adaptive') {
...
async function executeWithErrorHandling(cmd: string, execId: string) {
...
class MyCustomStrategy extends ExecutionStrategy {
  async execute(command: string, executionId: string) {
...
async function executeAdaptiveWithTracking(cmd: string, execId: string) {
...
class CommandExecutor {
  ...
  private factory: ExecutionStrategyFactory;
  ...
  constructor() {
    ...
    for (const mode of this.factory.getSupportedModes()) {
      this.strategies.set(mode, this.factory.createStrategy(mode as any, {
        timeoutMs: 300000,
        killGracePeriodMs: 5000,
        captureStderr: true,
        maxOutputSize: 10 * 1024 * 1024,
  ...
  async execute(cmd: string, mode: string, execId: string) {
