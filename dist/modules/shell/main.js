...
import { CallToolRequestSchema, ListToolsRequestSchema, ErrorCode, McpError, } from '@modelcontextprotocol/sdk/types.js'; // Adapted SDK import
...
import { FileManager } from '../../core/file-manager.js'; // Adapted import
import { MonitoringManager } from '../../core/monitoring-manager.js'; // Adapted import
import { SecurityManager } from '../../security/manager.js'; // Adapted import
import { McpShellConfigManager as ConfigManager } from '../../core/shell-config-manager.js'; // Adapted import and aliased
...
const DISABLED_TOOLS = (process.env['MCP_DISABLED_TOOLS'] || '')
    .split(',')
    .map((t) => t.trim())
    .filter((t) => t.length > 0);
export class MCPShellServer {
    constructor() {
        ...
        this.commandHistoryManager.loadHistory().catch(error => {
            logger.warn('Failed to load command history:', { error: error instanceof Error ? error.message : String(error) });
        });
        ...
        // ツールハンドラーの初期化
        this.shellTools = new ShellTools(this.processManager, this.terminalManager, this.fileManager, this.monitoringManager, this.securityManager, this.commandHistoryManager);
        this.setupHandlers();
    }
    setupHandlers() {
        // ツールリストの提供
        this.server.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                {
                    name: 'shell_execute',
                    description: 'Execute shell commands securely with intelligent output handling. When output_truncated=true, use output_id with read_execution_output to get complete results. Supports adaptive execution mode that automatically switches to background for long-running commands. New: Support pipeline operations with input_output_id to use previous command output as input. NOTE: This is MCP Shell Server tool - do NOT use VS Code internal run_in_terminal parameters like "explanation" or "isBackground".',
        ...
        this.server.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                logger.error(`Error handling tool request for ${request.params.name}: ${error instanceof Error ? error.message : String(error)}`);
                throw error;
            }
        });
    }
    // バックグラウンドプロセス通知用の共通ヘルパー
    getProcessNotificationInfo(executionInfo) {
    ...
    async notifyBackgroundProcessComplete(executionId, executionInfo) {
        ...
        try {
            ...
            logger.error(`Failed to send background process completion notification: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async notifyBackgroundProcessError(executionId, executionInfo, error) {
        const { commandPreview, outputSizeStr } = this.getProcessNotificationInfo(executionInfo);
        ...
        try {
            ...
            // 通知送信エラーは内部ログのみ（フォールバックとしてstderr出力）
            logger.error(`Failed to send background process error notification: ${notificationError instanceof Error ? notificationError.message : String(notificationError)}`);
        }
    }
    async notifyBackgroundProcessTimeout(executionId, executionInfo) {
        const { commandPreview, outputSizeStr } = this.getProcessNotificationInfo(executionInfo);
        ...
        try {
            ...
            // 通知送信エラーは内部ログのみ（フォールバックとしてstderr出力）
            logger.error(`Failed to send background process timeout notification: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async run() {
