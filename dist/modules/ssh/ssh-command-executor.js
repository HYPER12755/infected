        ...
        return CorrelationContext.runAsync(context, async () => {
            ...
            try {
                ...
                while (Date.now() - startTime < validTimeout) {
                    ...
                    if (!session.isConnected) {
                        session.isReady = true;
                        throw new SSHError(`Session ${session.id} disconnected during command execution`, { code: SSHErrorCode.DISCONNECTED, severity: ErrorSeverity.HIGH, retryable: true });
                    }
