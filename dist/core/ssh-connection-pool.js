...
export class SSHConnectionPool extends EventEmitter {
    ...
    async getConnection(options) {
        ...
        if (this.connectionCache.size >= this.config.maxConnections) {
            ...
            if (this.connectionCache.size >= this.config.maxConnections) {
                throw new Error(`SSH Connection Pool is at maximum capacity (${this.config.maxConnections} connections)`);
            }
        }
    ...
    releaseConnection(connectionId) {
        const connection = Array.from(this.connectionCache.values()).find((c) => c.connectionId === connectionId);
        if (!connection) {
    ...
    async closeConnection(connectionId) {
        ...
        try {
            ...
            // Remove from cache
            const cacheKey = Array.from(this.connectionCache.entries()).find(([_, v]) => v.connectionId === connectionId)?.[0];
            if (cacheKey) {
    ...
    getConnectionStats() {
        ...
        const avgConnectionAge = connections.length > 0
            ? connections.reduce((sum, c) => sum + (Date.now() - c.createdAt), 0) /
                connections.length
            : 0;
        const avgConnectionReuses = connections.length > 0
            ? connections.reduce((sum, c) => sum + c.useCount, 0) / connections.length
            : 0;
        const poolUtilization = this.config.maxConnections > 0
    ...
    async pruneStaleConnections() {
        ...
        await Promise.all(connectionsToPrune.map((connectionId) => this.closeConnection(connectionId)));
    ...
    async shutdown() {
        ...
        // Close all remaining connections
        const connectionIds = Array.from(this.connectionCache.values()).map((c) => c.connectionId);
     ...
     */
    hashCredentials(password, privateKey) {
     ...
     */
    async createConnection(options, cacheKey) {
 ...
 */
let globalPool;
export function getGlobalSSHConnectionPool(config) {
    if (!globalPool) {
 ...
 * @internal
 */
export function resetGlobalSSHConnectionPool() {
    if (globalPool) {
