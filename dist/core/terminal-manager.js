...
import { generateId, getCurrentTimestamp, getSafeEnvironment } from '../utils/shell-helpers.js'; // Adapted import
...
export class TerminalManager {
    constructor(maxTerminals = 20, maxOutputLines = 10000, maxHistoryLines = 1000) {
        ...
        this.terminalReadPositions = new Map(); // Track last read position for each terminal
    ...
    async createTerminal(options) {
        ...
        try {
            ...
            logger.error(`Failed to create terminal: ${error instanceof Error ? error.message : String(error)}`, {
                shellType: shellType,
                error: String(error),
            });
    ...
    handleTerminalExit(terminalId, _exitCode) {
        const session = this.terminals.get(terminalId);
        if (!session)
            return;
    ...
    async getTerminal(terminalId, updateForegroundProcess = true) {
        const session = this.terminals.get(terminalId);
        if (!session) {
    ...
    async sendInput(terminalId, input, execute = false, controlCodes = false, rawBytes = false, sendTo) {
        const session = this.terminals.get(terminalId);
        if (!session) {
            throw new ResourceNotFoundError('terminal', terminalId);
        }
        ...
        try {
            ...
            if (rawBytes) {
                ...
                try {
                    ...
                    logger.error('Invalid hex string for raw_bytes mode:', { error: error instanceof Error ? error.message : String(error) });
            ...
            this.updateForegroundProcess(session).catch((err) => {
                logger.warn(`Failed to update foreground process for terminal ${terminalId}:`, { error: err instanceof Error ? err.message : String(err) });
            });
            const result = {
                success: true,
            ...
            logger.error(`Failed to send input: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async getOutput(terminalId, startLine, lineCount = 100, includeAnsi = false, includeForegroundProcess = false) {
        const session = this.terminals.get(terminalId);
        if (!session) {
            throw new ResourceNotFoundError('terminal', terminalId);
        }
        // startLineが指定されていない場合は、前回の読み取り位置を使用
        const actualStartLine = startLine !== undefined ? startLine : this.terminalReadPositions.get(terminalId) || 0;
     ...
     */
    setReadPosition(terminalId, position) {
        const session = this.terminals.get(terminalId);
        if (!session) {
            throw new ResourceNotFoundError('terminal', terminalId);
        }
    ...
    resizeTerminal(terminalId, dimensions) {
        ...
        try {
            ...
            logger.error(`Failed to resize terminal: ${error instanceof Error ? error.message : String(error)}`);
            throw new ExecutionError(`Failed to resize terminal: ${error}`);
        }
    }
    closeTerminal(terminalId, saveHistory = true) {
        const session = this.terminals.get(terminalId);
        if (!session) {
            throw new ResourceNotFoundError('terminal', terminalId);
        }
        ...
        try {
            ...
            logger.error(`Failed to close terminal: ${error instanceof Error ? error.message : String(error)}`);
            throw new ExecutionError(`Failed to close terminal: ${error}`);
        }
    }
    // アイドル状態のターミナルの検出
    getIdleTerminals(idleMinutes = 30) {
        const now = new Date();
    ...
    cleanup() {
        ...
        for (const [terminalId] of this.terminals) {
            try {
                ...
                logger.error(`Failed to cleanup terminal ${terminalId}:`, { error: error instanceof Error ? error.message : String(error) });
            }
        }
     ...
     * フォアグラウンドプロセス情報を更新する
     */
    async updateForegroundProcess(session) {
        try {
            // キャッシュチェック（5秒間有効）
            const now = Date.now();
            ...
            logger.warn(`Failed to update foreground process for terminal ${session.info.terminal_id}:`, { error: error instanceof Error ? error.message : String(error) });
     ...
     * プログラムガードをチェックする
     */
    async checkProgramGuard(terminalId, sendTo) {
        if (sendTo === '*') {
            return true; // 条件なし
        }
        const session = this.terminals.get(terminalId);
