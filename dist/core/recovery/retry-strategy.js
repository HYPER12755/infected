...
export class RetryStrategy {
            ...
            try {
                ...
                if (retryContext.currentAttempt >= retryContext.maxAttempts) {
                    ...
                    if (typeof baseError.context === 'object' && baseError.context !== null) {
                        baseError.context.retryAttempts = retryContext.currentAttempt;
                        baseError.context.totalRetryDelayMs = retryContext.totalDelayMs;
                        baseError.context.retryExhausted = true;
                    }
                    throw baseError;
                }
     ...
     * ```
     */
    estimateMaxRetryTime() {
        let totalTime = 0;
        for (let attempt = 2; attempt <= this.config.maxAttempts; attempt++) {
            const delayWithoutJitter = this.config.initialDelayMs *
 ...
 * ```
 */
export function createRetryStrategy(preset = 'moderate') {
    const presets = {
        aggressive: {
            maxAttempts: 10,
            initialDelayMs: 50,
