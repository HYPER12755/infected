...
import { CommandHistoryEntrySchema, } from '../types/shell-server/enhanced-security.js'; // Adapted import
...
export class CommandHistoryManager {
    ...
    async loadHistory() {
        try {
            ...
            if (error.code === 'ENOENT') {
                ...
                logger.error(`Failed to load command history: ${error instanceof Error ? error.message : String(error)}`, { path: this.historyPath });
    ...
    async saveHistory() {
        try {
            ...
            logger.error(`Failed to save command history: ${error instanceof Error ? error.message : String(error)}`, { path: this.historyPath });
    ...
    predictUserConfirmation(command) {
        ...
        const matchingPatterns = this.userPatterns.filter((pattern) => {
            try {
                ...
                logger.debug(`Invalid regex pattern in user confirmation pattern: ${pattern.command_pattern}`, { error: error instanceof Error ? error.message : String(error) });
                return pattern.command_pattern === baseCommand;
            }
        });
    ...
    async cleanupOldEntries() {
        ...
        if (removedCount > 0) {
            logger.info(`Cleaned up ${removedCount} old command history entries`); // Use logger
        }
    }
    /**
