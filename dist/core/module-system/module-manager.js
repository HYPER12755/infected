import { getInstallRoot, getRuntimeMode, getWorkspaceRoot } from '../../config/index.js';
import logger from '../logger.js';
import * as path from 'node:path';
import * as url from 'node:url';
import * as fs from 'node:fs';
import { UnifiedModuleManifestSchema, } from './module-types.js';
...
export class ModuleManager extends EventEmitter {
    ...
    constructor(server, config, managers, toolCacheManager, permissionManager, monitoringManager) {
        super(); // Call EventEmitter constructor
        // Use module ID as key for loadedModules to prevent duplicates and easier access
        this.loadedModules = new Map();
        this.loadingModules = new Set(); // To prevent re-entry during hot reload cycles
        this.deregisterFunctions = new Map(); // K: toolId, V: registered tool metadata
        this.server = server;
        this.config = config;
    ...
    async initialScanAndLoad() {
        const moduleDirsToProcess = new Set();
        const addDirIfReadable = (dirPath) => {
            try {
                fs.accessSync(dirPath, fs.constants.R_OK);
        ...
        const firstReadableDir = (candidates) => {
    ...
    resolveModuleRootFromFile(fullPath, rootWatchDir) {
        let bestMatch;
    ...
    getModuleTypeFromPath(fullPath) {
    ...
    isLegacyModule(moduleCandidate) {
        return !!moduleCandidate && typeof moduleCandidate.register === 'function';
    }
    createLegacyModuleAdapter(moduleDirPath, legacyModule) {
    ...
    async loadModuleFromDirectory(moduleDirPath) {
        ...
        try {
            ...
            if (parsedManifest.success) {
                ...
                if (manifest.entry) {
                    ...
                    try {
                        await fs.promises.access(moduleEntryFile);
                    }
                    catch {
                        moduleEntryFile = path.join(moduleDirPath, 'index.ts');
                    }
                    try {
                        await fs.promises.access(moduleEntryFile);
                    }
                    catch {
                        moduleEntryFile = null;
                    }
                }
            }
            else {
        ...
        if (isProductionInstallModule && moduleEntryFile?.endsWith('.ts')) {
            ...
            try {
                ...
                logger.warn(`ModuleManager: Skipping TypeScript entry in production for ${moduleDirPath}. Expected transpiled JS entry at ${jsFallback}.`);
                this.loadingModules.delete(moduleDirPath);
                return;
            }
        }
        ...
        try {
            ...
            if (!manifest) {
                if (!moduleInstance.manifest) {
                    if (this.isLegacyModule(moduleInstance)) {
                        moduleInstance = this.createLegacyModuleAdapter(moduleDirPath, moduleInstance);
    ...
    async unloadModuleFromDirectory(moduleDirPath) {
    ...
    async reloadModuleFromDirectory(moduleDirPath) {
    ...
    registerToolExecution(toolId, executeFn, name, description = '', inputSchema = {}, moduleId, skipTimeout = false) {
        ...
        const logToolInvocation = (label, args, result) => {
        ...
        const permissionAndCachedAndMonitoredExecute = async (args) => {
