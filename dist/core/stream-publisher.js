...
export class StreamPublisher {
    constructor(options = {}) {
        ...
        this.executionSubscribers = new Map(); // execution -> subscriber IDs
        this.options = {
    ...
    subscribeToExecution(executionId, subscriberId) {
        if (!this.subscribers.has(subscriberId)) {
            throw new Error(`StreamPublisher: Subscriber ${subscriberId} not found`);
        }
        ...
        const notifications = Array.from(subscriberIds).map(async (subscriberId) => {
            ...
            if (subscriber) {
                try {
                    ...
                    logger.error(`StreamPublisher: Error notifying subscriber ${subscriberId}:`, { error: error instanceof Error ? error.message : String(error) });
                }
            }
        });
