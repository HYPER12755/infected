    ...
    async runCommand(executionId, command, opts) {
            ...
            try {
                await new Promise((resolve, reject) => {
                    ...
                    const endSafely = () => {
                        ...
                        catch (e) { /* stream may already be closed */
                            logger.debug(`ExecutorServer: Stdin end error for ${executionId}: ${e instanceof Error ? e.message : String(e)}`, { error: e });
                        }
                        resolve();
                    };
                ...
                try {
                    ...
                    logger.debug(`ExecutorServer: Error killing child process after input write failure for ${executionId}: ${killErr instanceof Error ? killErr.message : String(killErr)}`, { error: killErr });
                }
            }
        }
        let stdout = '';
        ...
        const timer = setTimeout(() => {
            ...
            try {
                ...
                logger.debug(`ExecutorServer: Error sending SIGTERM to ${executionId}: ${e instanceof Error ? e.message : String(e)}`, { error: e });
            }
            setTimeout(() => {
                if (!child.killed) {
                    ...
                    try {
                        ...
                        logger.debug(`ExecutorServer: Error sending SIGKILL to ${executionId}: ${e instanceof Error ? e.message : String(e)}`, { error: e });
                    }
                }
            }, 1000);
