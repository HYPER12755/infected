...
async function createBackup(config) {
    try {
        ...
        logger.error('Failed to create backup:', { error: error });
        throw error;
    }
}
/**
...
export async function loadCriteria() {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            logger.info('Criteria file not found, using default criteria.');
            return config.defaultContent;
        }
        logger.error('Failed to load criteria:', { error: error });
        throw error;
    }
}
/**
    ...
    if (appendMode) {
        try {
            ...
            if (error.code === 'ENOENT') {
                ...
                logger.error('Failed to read existing criteria for append mode:', { error: error });
                throw error;
            }
        }
    }
...
export async function isCriteriaModified(lastModified) {
    ...
    try {
        ...
        logger.error('Failed to check if criteria was modified:', { error: error });
        throw error;
    }
}
/**
...
export async function getCriteriaStatus() {
    ...
    try {
        ...
        logger.error('Failed to get criteria status:', { error: error });
        throw error;
    }
}
/**
    ...
    try {
        ...
        if (backupExisting) {
            try {
                ...
                logger.warn('Failed to create backup before criteria adjustment', { error: error });
            }
        }
        // Prepare new content
        let newContent;
