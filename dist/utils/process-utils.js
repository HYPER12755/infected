import * as fs from 'node:fs'; // Use node:fs
...
export class ProcessUtils {
    ...
    static async getProcessInfo(pid) {
        ...
        try {
            ...
            logger.error(`Failed to get process info for PID ${pid}:`, { error: error });
            return null;
        }
    }
    /**
     ...
     */
    static async getForegroundProcess(terminalPty) {
        try {
            // pty の file descriptor を取得
            const ptyWithFd = terminalPty;
            ...
            try {
                ...
                logger.error('Failed to get foreground process:', { error: error });
            }
            if (!foregroundPid) {
                return {
                    available: false,
            ...
            return {
                available: false,
                error: `Error getting foreground process: ${error instanceof Error ? error.message : String(error)}`,
            };
        }
    }
    /**
     ...
     */
    static async findLatestChildProcess(parentPid) {
        try {
            const procDir = '/proc';
            ...
            for (const entry of entries) {
                ...
                try {
                    ...
                    logger.debug(`Skipping process entry ${entry} due to error: ${error}`, { error: error });
                    // このプロセスはスキップ
                    continue;
                }
            }
            ...
            logger.error('Error finding child processes:', { error: error });
            return null;
        }
    }
    /**
     * プログラムガードの条件をチェックする
     */
    static checkProgramGuard(processInfo, sendTo) {
        if (sendTo === '*') {
            return true; // 条件なし
        }
        if (!processInfo) {
...
ProcessUtils.processInfoCache = new Map();
ProcessUtils.CACHE_TTL = 1000; // 1秒のキャッシュ
