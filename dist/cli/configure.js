...
import { InfectedConfigSchema } from '../config/schema.js';
...
export async function configureCLI() {
    ...
    const generalAnswers = await inquirer.prompt([
        ...
        },
        {
            ...
            when: (answers) => answers.transport === 'http' || answers.transport === 'sse',
        },
        {
            type: 'confirm',
            name: 'hotReload',
    ...
    const pluginAnswers = await inquirer.prompt([
        ...
        },
        {
            ...
            when: (answers) => answers.configurePlugins,
            filter: (value) => value.split(',').map(s => s.trim()).filter(s => s.length > 0).map(name => ({ name, config: {} })),
    ...
    const toolsDirAnswers = await inquirer.prompt([
        {
            ...
            validate: (value) => {
                if (!value.trim())
                    return chalk.red('Tool directory cannot be empty.');
                return true;
            },
    ...
    const authAnswers = await inquirer.prompt([
        ...
        },
        {
            ...
            when: (answers) => answers.authEnabled && !answers.randomAuthTokenEnabled,
            validate: (value) => {
                const keys = value.split(',').map(s => s.trim()).filter(s => s.length > 0);
                if (keys.length === 0)
                    return chalk.red('Please provide at least one API key.');
                if (keys.some(key => key.length < 8))
                    return chalk.red('Each API key must be at least 8 characters long.');
                return true;
            },
            filter: (value) => value.split(',').map(s => s.trim()).filter(s => s.length > 0)
        }
    ]);
    ...
    const permissionsAnswers = await inquirer.prompt([
        ...
        },
        {
            ...
            filter: (value) => value.split(',').map(s => s.trim()).filter(s => s.length > 0),
            when: (answers) => answers.defaultPolicy === 'deny' // Only relevant if default is deny
        },
        {
            type: 'input',
            name: 'toolBlocklist',
        ...
        },
        {
            ...
            validate: (value) => {
                ...
                if (isNaN(ttl) || ttl < 1000)
                    return chalk.red('Please enter a minimum of 1000ms.');
                return true;
            },
            filter: (value) => parseInt(value),
    ...
    const llmSecurityAnswers = await inquirer.prompt([
        ...
        },
        {
            ...
            when: (answers) => answers.llmSecurityEnabled,
            validate: (value) => value.trim() ? true : chalk.red('Provider is required if LLM security is enabled.'),
        },
        {
            type: 'input',
            ...
            when: (answers) => answers.llmSecurityEnabled,
            validate: (value) => value.trim() ? true : chalk.red('Model is required if LLM security is enabled.'),
        },
        {
            type: 'input',
            ...
            when: (answers) => answers.llmSecurityEnabled,
            validate: (value) => value.trim() ? true : chalk.red('API Key is required if LLM security is enabled.'),
