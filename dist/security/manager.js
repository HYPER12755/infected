import { DEFAULT_ENHANCED_SECURITY_CONFIG, DEFAULT_BASIC_SAFETY_RULES, } from '../types/shell-server/enhanced-security.js'; // Adapted import
...
const DEFAULT_LLM_SECURITY_CONFIG = {
    enabled: false,
    provider: undefined,
    model: undefined,
    apiKey: undefined,
...
export class SecurityManager {
    constructor(mcpShellConfig, llmSecurityConfig) {
    ...
    setConfig(mcpShellConfig, llmSecurityConfig) {
    ...
    consolidateLlmSettings() {
        ...
        if (this.llmSecurityConfig.provider) {
            this.enhancedConfig.llm_provider = this.llmSecurityConfig.provider;
        }
    ...
    initializeEnhancedEvaluator(historyManager, server) {
    ...
    async evaluateCommandSafetyByEnhancedEvaluator(command, workingDirectory, comment, forceUserConfirm) {
        ...
        logger.debug(`Enhanced Evaluator - History commands: ${history.map((h) => h.command).join(', ')}`);
