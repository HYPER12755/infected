...
export class CCCToMCPCMAdapter {
    ...
    async tryParseDirectToolCalls(responseText) {
        try {
            ...
            try {
                ...
                logger.debug('JSON repair failed for direct tool calls parse attempt.', { error: repairError instanceof Error ? repairError.message : String(repairError) });
            }
        }
        return null;
    }
    ...
    extractJsonObjects(text) {
        ...
        try {
            ...
            logger.debug('Failed to parse entire text as JSON, trying extraction.', { error: e instanceof Error ? e.message : String(e) });
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (jsonText) {
                    try {
                        ...
                        logger.debug('Failed to parse extracted JSON text:', { jsonText, error: e instanceof Error ? e.message : String(e) });
 ...
 */
function createMessageCallbackFromMCPServer(server) {
    return async (request) => {
        try {
            // Convert request to MCP format
            const mcpMessages = request.messages
                .filter((msg) => msg.role !== 'tool' && msg.content?.type === 'text')
                .map((msg) => ({
                role: msg.role,
            ...
            const contentText = result.content && result.content.type === 'text' ? result.content.text : '';
            const response = {
            ...
            logger.error('Error in createMessageCallbackFromMCPServer:', { error: error instanceof Error ? error.message : String(error) });
            throw error;
        }
    };
}
