...
export class SSHFileTransferHandler {
  ...
  async uploadFile(
    ...
    return CorrelationContext.runAsync(context, async () => {
      ...
      try {
        ...
        try {
          const dirCheck = await this.commandExecutor.executeCommand(
            session,
        ...
        try {
          const fileStatus = await this.commandExecutor.executeCommand(
            session,
        ...
        if (verify.exitCode !== 0) {
          ...
          throw new Error(
            `File upload verification failed. Remote file may not have been created: ${output || 'File not found'}`
          );
        }

        if (!verify.output.includes(path.basename(finalRemotePath))) {
          throw new Error(
            `File upload verification failed. Remote file may not have been created: ${verify.output || 'File not found'}`
          );
        }
      ...
      try {
        ...
        if (sizeResult.exitCode !== 0) {
          ...
          if (
            ...
            throw new Error(
              `Permission denied accessing remote file: ${remotePath}. Check read permissions for the file.`
            );
          }
   ...
   */
  async deleteRemoteFile(session: Session, remotePath: string, timeout: number = 10000): Promise<void> {
    const context = CorrelationContext.generate(undefined, undefined, session.id);
  ...
  private async runLocalCommand(command: string, timeout: number) {
    ...
    if (!this.isCommandAvailable(commandName)) {
      throw new Error(
        `Required command '${commandName}' not found. Please ensure OpenSSH (scp, sftp) is installed on the system.`
      );
    }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      proc.on('error', (error) => {
        ...
        if (error.message.includes('ENOENT')) {
          reject(new Error(
            `Required command '${commandName}' not found. Please ensure OpenSSH (scp, sftp) is installed.`
          ));
          return;
        }
        reject(error);
   ...
   */
  resolveRemotePath(remotePath: string, session?: { target?: SSHConnectionTarget }): string {
    if (session?.target) {
      return remotePath;
    }
