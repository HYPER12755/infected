...
/**
 * Information about detected prompt
 */
...
export class SSHPromptDetector {
   ...
   */
  detectPrompt(output: string): PromptInfo {
    const patterns = [
   ...
   */
  isPromptDetected(sessionId: string, session: Session): boolean {
   ...
   */
  detectInteractivePrompts(output: string): { detected: boolean; type?: string; prompt?: string } {
   ...
   */
  getPromptInfo(sessionId: string, output: string): PromptInfo {
