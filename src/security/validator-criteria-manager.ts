...
export class ValidatorCriteriaManager {
  ...
  public async adjustCriteria(criteriaText: string, backupExisting: boolean = true): Promise<{
    ...
    try {
      ...
      logger.error('Failed to adjust validator criteria', { error: error instanceof Error ? error.message : String(error) });
      return {
        success: false,
  ...
  public async loadCriteria(): Promise<string | null> {
    try {
      ...
      logger.error('Failed to load validator criteria', { error: error instanceof Error ? error.message : String(error) });
      return null;
    }
  }
  ...
  public async getCriteriaConfig(): Promise<ValidatorCriteriaConfig | null> {
    try {
      ...
      logger.error('Failed to get validator criteria config', { error: error instanceof Error ? error.message : String(error) });
      return null;
    }
  }
  ...
  private getLatestBackupPath(): string | undefined {
    try {
      ...
      logger.warn('Failed to find latest backup', { error: error instanceof Error ? error.message : String(error) });
      return undefined;
    }
  }
  ...
  private ensureDirectoryExists(dirPath: string): void {
    try {
      ...
      logger.error('Failed to create validator criteria directory', { dirPath, error: error instanceof Error ? error.message : String(error) });
    }
  }
