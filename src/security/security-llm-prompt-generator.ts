...
import { SafetyLevel } from '../types/shell-server/enhanced-security.js'; // Adapted import
import { loadCriteria } from '../utils/criteria-manager.js'; // Adapted import
...
## TOOL-FIRST MODE (MANDATORY)
You MUST respond with exactly one tool call: allow(), deny(), add_more_history(), user_confirm(), or ai_assistant_confirm().
Never return plain text as the final answer.
If context is insufficient, prefer ai_assistant_confirm() or add_more_history() instead of guessing.
...
- Did I choose exactly one tool?
- Did I provide only the parameters required by that tool?
- Did I avoid returning plain text as the final output?
- If uncertain, did I choose ai_assistant_confirm() or add_more_history()?`;
...
**MANDATORY INSTRUCTION**: Return exactly one tool call using evaluate_command_security. Do not return plain text.`;
