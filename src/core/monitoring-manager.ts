import { MonitorInfo, SystemStats } from '../types/shell-server/index.js'; // Adapted import
...
interface DashboardMetrics {
  ...
  toolExecution: {
    ...
    failedCalls: number;
    successRate: number;
    recentlyExecuted: Array<{ toolName: string; durationMs: number; success: boolean; timestamp: string; }>;
  };
  cache: {
...
interface ToolExecutionRecord {
  ...
  isCached: boolean; // Whether the result was served from cache
}
...
export class MonitoringManager {
  ...
  private readonly maxToolExecutionHistory = 100;

  // Dependencies that will be set after construction
  private toolLoader: ToolLoader | undefined;
  private toolCacheManager: ToolCacheManager | undefined;
  private _toolExecutionRecords: ToolExecutionRecord[] = [];
   ...
   */
  public setToolManagers(toolLoader: ToolLoader, toolCacheManager: ToolCacheManager): void {
   ...
   */
  reportToolExecution(toolName: string, durationMs: number, success: boolean, isCached: boolean): void {
