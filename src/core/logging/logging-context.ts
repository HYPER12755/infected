...
/**
 * Logging context metadata interface
 */
export interface ILoggingMetadata {
  [key: string]: any;
}
...
export class LoggingContext {
   ...
   */
  private mergeMetadata(userMeta?: ILoggingMetadata): Record<string, any> {
   ...
   */
  error(message: string, meta?: ILoggingMetadata): void {
   ...
   * @param meta - Optional metadata to include
   */
  warn(message: string, meta?: ILoggingMetadata): void {
    const merged = this.mergeMetadata(meta);
   ...
   * @param meta - Optional metadata to include
   */
  info(message: string, meta?: ILoggingMetadata): void {
    const merged = this.mergeMetadata(meta);
   ...
   * @param meta - Optional metadata to include
   */
  debug(message: string, meta?: ILoggingMetadata): void {
    const merged = this.mergeMetadata(meta);
   ...
   * @param meta - Optional metadata to include
   */
  http(message: string, meta?: ILoggingMetadata): void {
    const merged = this.mergeMetadata(meta);
   ...
   * @param meta - Optional metadata to include
   */
  fatal(message: string, meta?: ILoggingMetadata): void {
    const merged = this.mergeMetadata(meta);
    (logger as any).fatal(message, merged);
  }
  ...
  async withChildContextAsync<T>(
    ...
    try {
      const childContext = CorrelationContext.createChild(userId, sessionId);
      return await this.withContextAsync(
        childContext,
        callback,
        operation
      );
