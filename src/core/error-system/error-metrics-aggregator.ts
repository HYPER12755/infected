...
import type { ErrorMetricsSnapshot, ErrorTrend, CategoryHealth } from './error-metrics.js';
import type { ErrorHealthCheck, HealthReport } from './error-health-check.js';
import type { RecoveryMetrics } from '../recovery/recovery-handler.js';
...
export interface ResourceMetrics {
  ...
  /** File handle limit (if set) */
  fileHandleLimit?: number;
  /** Active connections count */
  activeConnections: number;
  /** Connection limit (if set) */
...
export interface ComponentMetrics {
  ...
  name: string;
  /** Component type */
  type: 'error' | 'recovery' | 'circuitBreaker' | 'resource';
  /** Health score 0-100 */
  healthScore: number;
  /** Specific metrics for this component */
...
export interface AggregatedMetrics {
  ...
  /** Recovery handler metrics */
  recoveryMetrics?: RecoveryMetrics;
  /** Circuit breaker metrics by name */
  circuitBreakerMetrics: Record<string, CircuitBreakerMetrics & { state: CircuitState }>;
  /** Resource metrics */
  ...
  /** Components included in aggregation */
  components: ComponentMetrics[];
}

/**
 * Anomaly in system metrics
 */
export interface Anomaly {
  /** Type of anomaly detected */
  type: 'error-rate-spike' | 'recovery-failure-increase' | 'circuit-breaker-state-change' | 'resource-warning';
  /** Severity: 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL' */
  severity: string;
  /** Description of the anomaly */
  description: string;
  /** Component or category affected */
...
export class ErrorMetricsAggregator {
  private healthCheck: ErrorHealthCheck;
  private errorMetrics: { getMetrics: () => ErrorMetricsSnapshot; getTrendAnalysis: () => ErrorTrend[]; getCategoryHealthSummary: () => Record<ErrorCategory, CategoryHealth> };
  private recoveryHandler?: { getRecoveryMetrics: () => RecoveryMetrics };
  private circuitBreakers: Map<string, { getMetrics: () => CircuitBreakerMetrics; getState: () => CircuitState }>;
  private resourceMetrics?: { getMetrics: () => ResourceMetrics };
  private lastAggregation: AggregatedMetrics | null = null;
  private anomalyHistory: Anomaly[] = [];
  private maxAnomalyHistory = 100;
  ...
  constructor(
    ...
    errorMetrics: { getMetrics: () => ErrorMetricsSnapshot; getTrendAnalysis: () => ErrorTrend[]; getCategoryHealthSummary: () => Record<ErrorCategory, CategoryHealth> }
  ) {
   ...
   * ```
   */
  async collectMetrics(): Promise<AggregatedMetrics> {
    ...
    const circuitBreakerMetrics: Record<string, CircuitBreakerMetrics & { state: CircuitState }> = {};
  ...
  getSystemHealthScore(): number {
    if (!this.lastAggregation) return 100;
   ...
   */
  getMetricsByComponent(): ComponentMetrics[] {
   ...
   * @returns Array of detected anomalies
   */
  getAnomalyHistory(limit: number = 20): Anomaly[] {
   ...
   */
  registerRecoveryMetrics(recoveryHandler: { getRecoveryMetrics: () => RecoveryMetrics }): void {
   ...
   */
  registerResourceMetrics(resourceMonitor: { getMetrics: () => ResourceMetrics }): void {
   ...
   */
  getLastMetrics(): AggregatedMetrics | null {
   ...
   */
  private calculateResourceScore(resourceMetrics: ResourceMetrics): number {
    let score = 100;

    // Memory penalty
    if (resourceMetrics.memoryLimitMB) {
      const memoryPercent = (resourceMetrics.memoryUsageMB / resourceMetrics.memoryLimitMB) * 100;
      if (memoryPercent > 90) score -= 40;
      else if (memoryPercent > 75) score -= 20;
      else if (memoryPercent > 50) score -= 10;
    }

    // CPU penalty
    if (resourceMetrics.cpuUsagePercent > 90) score -= 30;
    else if (resourceMetrics.cpuUsagePercent > 75) score -= 15;
    else if (resourceMetrics.cpuUsagePercent > 50) score -= 5;
    ...
    if (resourceMetrics.fileHandleLimit) {
      ...
      if (handlesPercent > 90) score -= 30;
      else if (handlesPercent > 75) score -= 15;
    }
    ...
    if (resourceMetrics.connectionLimit) {
      ...
      if (connPercent > 90) score -= 25;
      else if (connPercent > 75) score -= 12;
    }
