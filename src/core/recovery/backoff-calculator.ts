...
/**
 * Parameters for backoff calculations
 */
export interface BackoffParams {
  ...
  increment?: number;
  /** Degree for polynomial backoff */
  degree?: number;
  /** Maximum delay cap in milliseconds */
  maxDelay: number;
  /** Whether to apply jitter (default: true) */
  useJitter?: boolean;
  /** Jitter fraction as decimal (0.1 = 10%, default: 0.1) */
...
export class BackoffCalculator {
   ...
   */
  public static decorrelatedJitter(
   ...
   */
  public static addJitter(delay: number, jitterFraction: number = 0.1): number {
   ...
   */
  public static addFullJitter(baseDelay: number, maxDelay: number): number {
   ...
   */
  public static addEqualJitter(delay: number, baseDelay: number): number {
   ...
   */
  public static addDecorrelatedJitter(
    previousDelay: number,
    baseDelay: number,
    maxDelay: number
  ): number {
   ...
   */
  public static validateBackoffParams(params: Partial<BackoffParams>): boolean {
    const {
      baseDelay = 0,
      maxDelay = 0,
      multiplier,
   ...
   */
  public static estimateMaxBackoff(
    attempts: number,
    params: Partial<BackoffParams>
  ): number {
 ...
 */
export const BACKOFF_PRESET_AGGRESSIVE: BackoffParams = {
  baseDelay: 50,
  multiplier: 2,
  maxDelay: 5000,
  useJitter: true,
 ...
 */
export const BACKOFF_PRESET_MODERATE: BackoffParams = {
  baseDelay: 100,
  multiplier: 2,
  maxDelay: 30000,
  useJitter: true,
 ...
 */
export const BACKOFF_PRESET_CONSERVATIVE: BackoffParams = {
  baseDelay: 500,
  increment: 500,
  maxDelay: 60000,
  useJitter: true,
 ...
 */
export const BACKOFF_PRESET_CONSTANT: BackoffParams = {
  baseDelay: 10,
  maxDelay: 10,
  useJitter: false,
};
